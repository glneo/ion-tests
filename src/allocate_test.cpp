...
TEST_F(Allocate, Allocate)
{
	...
	for (unsigned int heapMask : m_allHeaps) {
		for (size_t size : allocationSizes) {
			...
			int handleFd = -1;
			ASSERT_EQ(0, ion_alloc(m_ionFd, size, heapMask, 0, &handleFd));
			ASSERT_TRUE(handleFd >= 0);
			ASSERT_EQ(0, ion_free(m_ionFd, handleFd));
		}
	}
}
...
TEST_F(Allocate, AllocateCached)
{
	...
	for (unsigned int heapMask : m_allHeaps) {
		for (size_t size : allocationSizes) {
			...
			SCOPED_TRACE(::testing::Message() << "size " << size);
			int handleFd = -1;
			ASSERT_EQ(0, ion_alloc(m_ionFd, size, heapMask, ION_FLAG_CACHED, &handleFd));
			ASSERT_TRUE(handleFd >= 0);
			ASSERT_EQ(0, ion_free(m_ionFd, handleFd));
		}
	}
...
TEST_F(Allocate, Zeroed)
{
	...
	for (unsigned int heapMask : m_allHeaps) {
		...
		for (unsigned int i = 0; i < 16; i++) {
			...
			ASSERT_EQ(0, ion_alloc(m_ionFd, 4096, heapMask, 0, &map_fd));
			ASSERT_GE(map_fd, 0);
		...
		ASSERT_EQ(0, ion_alloc(newIonFd, 4096, heapMask, 0, &map_fd));
		ASSERT_GE(map_fd, 0);
...
TEST_F(Allocate, Large)
{
	for (unsigned int heapMask : m_allHeaps) {
		SCOPED_TRACE(::testing::Message() << "heap " << heapMask);
		int handleFd = -1;
		ASSERT_EQ(-ENOMEM, ion_alloc(m_ionFd, 3UL * 1024 * 1024 * 1024, heapMask, 0, &handleFd));
	}
}
